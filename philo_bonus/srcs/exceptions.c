...
/*   Updated: 2025/08/25 22:58:11 by galauren         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
\"I must eat before they do, or I might die tonight\".\n");
	...
	philo_status_print(p, " In the meantime could you check how much \
philosophers you wanted around that dinner table please ?\n\n\n");
	pthread_mutex_lock(&(p->tblptr->print_lock));
	pthread_mutex_lock(&(p->tblptr->death_lock));
	if (!p->tblptr->stop_it)
		printf("%ldms :\t[ %d ]  %d ?\n\n\n",
	...
	philo_status_print(p,
		"I feel like something has got out of hand so ill just leave\n");
	philo_status_print(p,
		"Next time try with less or equal to 5000 philosophers.\n");
